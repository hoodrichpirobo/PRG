package pract2;

/**
 * Clas PRGString: class of tools with methods for working with Strings.
 * 
 * @author PRG - ETSINF - DSIC - UPV
 * @version Academic Year 2023/2024
 */
public class PRGString {
    
    /** Objects of this class cannot be created. */
    private PRGString() { }
    
    /**
     * Returns the number of occurrences of the letter 'a' in the String given as parameter.
     * @param s String to be explored to find occurrences of the letter 'a'.
     * @return int
     */
    public static int countA(String s) {
        // Trivial case: Empty String
        if (s.length() == 0) { return 0; }
        // General case: Non empty String. Check with the next substring.
        else if (s.charAt(0) == 'a') { return 1 + countA(s.substring(1)); }
        else { return countA(s.substring(1)); }
    }

    /**
     * Returns the number of occurrences of the letter 'a' in
     * the String given as parameter from the specified position.
     * @param s String to be explored to find occurrences of the letter 'a'.
     * @param pos starting position of 's' where the substring starts.
     * @return int
     * PRECONDITION: pos >= 0
     */
    public static int countA(String s, int pos) {
        // Trivial case: Empty string
        if (pos >= s.length()) { return 0; }
        // General case: Non empty String. Check with the next substring.
        else if (s.charAt(pos) == 'a') { return 1 + countA(s, pos + 1); }
        else { return countA(s, pos + 1); }
    }

    /**
     * Returns the number of occurrences of the letter 'a' in
     * @param s String to be explored to find occurrences of the letter 'a'.
     * @return int
     */
    public static int countA2(String s) {
        // Trivial case: Empty String
        if (s.length() == 0) { return 0; }
        // General case: Non emtpy String. Check with the next substring.
        else if (s.charAt(s.length() - 1) == 'a') {
            return 1 + countA2(s.substring(0, s.length() - 1));
        } else { return countA2(s.substring(0, s.length() - 1)); }
    }

    /**
     * Check whether 'a' is prefix of 'b'.
     * 
     * This method recursively checks if the first string 'a' is a prefix of the second string 'b'.
     * 
     * Preconditions>
     * - 'a' and 'b' must be non-null strings. The behavior is undefined if null strings are passed.
     * - 'a' and 'b' can be of any length, including empty strings.
     * 
     * @param a The string to check if it is a prefix.
     * @param b The string to check against.
     * @return true if 'a' is a prefix of 'b', false otherwise.
     * -- TO BE COMPLETED --
     */
    public static boolean isPrefix(String a, String b) {
        /* TO BE COMPLETED */
        // Base case: if 'a' is empty, it's considered a prefix of 'b'
        if(a.isEmpty()){
            return true;
        }
        
        // Base case: if 'b' is empty or 'a' is longer than 'b', 'a' can't be a prefix
        if(b.isEmpty() || a.length() > b.length()){
            return false;
        }
        
        // Base case: if the first characters of 'a' and 'b' don't match, 'a' is not a prefix
        if(a.charAt(0) != b.charAt(0)){
            return false;
        }
        
        //Recursive case: check the remainder of 'a' and 'b'
        return isPrefix(a.substring(1), b.substring(1));
    }

    /**
     * Check whether 'a' is a substring of 'b'.
     * This method recursively checks if the first string 'a' is contained within the second string 'b',
     * by utilizing the previously defined `isPrefix` method.
     * 
     * Preconditions:
     * - 'a' and 'b' must be non-null strings. The behavior is undefined for null inputs.
     * - This method uses `isPrefix(String, String)` to check if 'a' is a prefix of any substring of 'b'.
     *
     * @param a The string to check if it is contained within 'b'.
     * @param b The string to be searched.
     * @return true if 'a' is a substring of 'b', false otherwise.
     */
    public static boolean isSubstring(String a, String b) {
        // Base case: if 'a' is empty, it's considered a substring of 'b'
        if (a.isEmpty()) {
            return true;
        }
        // Base case: if 'b' is empty, 'a' can't be a substring of 'b'
        if (b.isEmpty()) {
            return false;
        }
        // If 'a' is a prefix of 'b', then it is also a substring
        if (isPrefix(a, b)) {
            return true;
        }
        // Recursive case: Check if 'a' is a substring of the rest of 'b'
        return isSubstring(a, b.substring(1));
    }

}